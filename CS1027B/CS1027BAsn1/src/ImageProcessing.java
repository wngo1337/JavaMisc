		...
		try {
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					...
					utilityArray = new String[parameters.length - 1];
					for (int i = 0; i < parameters.length - 1; i++) {
						utilityArray[i] = parameters[i];
						System.out.println(utilityArray[i]);
					}
					
					parameters = utilityArray;
					//	 BASICALLY WE GOT A FUCKING PROBLEM BECAUSE MY LAPTOP USER PATH HAS A SPACE IN IT
