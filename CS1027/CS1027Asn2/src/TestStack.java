	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 1; i <= 37; ++i) {
				...
				if ((i == 27) && (s.length() != 60)) {
					testPassed = false;
					System.out.println("test 5 fail cuz length ain't right on first run");
					break;
				}
				else if ((i == 37) && (s.length() != 20)) {
					testPassed = false;
					System.out.println("test 5 fail cuz length ain't right on second run");
					break;
				}
			}
		} catch (Exception e) {
			testPassed = false;
			System.out.println("test 5 fail cuz ur dumb af and exception occurred. probably cuz stack "
					+ "says it is empty");
			}
