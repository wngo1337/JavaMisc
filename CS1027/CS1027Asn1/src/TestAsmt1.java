	...
	public static void main(String[] args) {
		...
		try {
			...
			if ((pos.getRow() != 5) || (pos.getCol() != 7)) {
				testPassed = false;
				System.out.println("test 1 failed cuz you didn't return the right stuff idiot");
			}
			pos.setRow(1);
			pos.setCol(2);
			if ((pos.getRow() != 1) || (pos.getCol() != 2)) { 
				testPassed = false;
				System.out.println("test 1 failed because your stuff doesn't set properly dumbass");
			}
			...
			if (!pos.equals(pos2)) {
				testPassed = false;
				System.out.println("test 1 failed cuz the equality check doesn't work idiot");
			}
			if (pos.equals(pos3)) {
				testPassed = false;
				System.out.println("test 1 failed cuz unequal positions are seen as equal dumbfuk");
			}
			if (testPassed)
		...
		// Test 4. Test method getSnake of class GameBoard and methods getLength and snakePosition of class Snake
			...
			if (theSnake.getLength() != 1) {
				testPassed = false;
				System.out.println("test 4 failed because the snake's length isn't right idiot");
			}
			pos = theSnake.getPosition(0);
			if (pos.getRow() != 2 || pos.getCol() != 2) {
				testPassed = false;
				System.out.println("test 4 failed cuz getPosition doesn't work stupid fuk");
			}
			if (testPassed)
		...
		// Test 6. Test newHeadPosition, snakePosition
			...
			if (!theSnake.newHeadPosition("up").equals(up)) {
				testPassed = false;
				System.out.println("test 6 failed cuz up doesn't return right head position");
			}
			if (!theSnake.newHeadPosition("down").equals(down)) {
				testPassed = false;
				System.out.println("test 6 failed cuz down doesn't return right head position");
			}
			if (!theSnake.newHeadPosition("left").equals(left)) {
				testPassed = false;
				System.out.println("test 6 failed cuz left doesn't return right head position");
			}
			if (!theSnake.newHeadPosition("right").equals(right)) {
				testPassed = false;
				System.out.println("test 6 failed cuz right doesn't return right head position");
			}
		...
		// Test grow 
			...
			if (theSnake.getLength() != 5) {
				testPassed = false;
				System.out.println("test 8 fail cuz wrong length dumbass");
			}
			if (!theSnake.snakePosition(new Position(1,2))) {
				testPassed = false;
				System.out.println("test 8 fail cuz grow right didn't work idiot");
			}
			if (!theSnake.snakePosition(new Position(2,2))) {
				testPassed = false;
				System.out.println("test 8 fail cuz grow down didn't work idiot");
			}
			if (!theSnake.snakePosition(new Position(3,2))) {
				testPassed = false;
				System.out.println("test 8 fail cuz grow down again didn't work idiot");
			}
			if (!theSnake.snakePosition(new Position(3,1))) {
				testPassed = false;			
				System.out.println("test 8 fail cuz grow left didn't work idiot");
			}
			if (testPassed)
